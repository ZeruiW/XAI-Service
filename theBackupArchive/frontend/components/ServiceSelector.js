	...
	return (
		<div
			...
			className={
				...
				(selection[svc._id] === 1 ? "bg-green-300" : "hover:bg-neutral-300")
			}
		>
...
export default function ServiceSelector({ setSvc }) {
	...
	return (
		<aside id="service-selector" className="w-64">
			<div className="overflow-y-auto rounded bg-gray-100 m-4 w-full">
				{services.map((s, idx) => (
					<ServiceCard
						key={idx}
						selection={selection}
						onSelection={() => handleChooseSvc(s._id)}
						svc={s}
					></ServiceCard>
				))}
			</div>
